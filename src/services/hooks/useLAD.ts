...
export function useCourseSummary(courseId: string, studentId?: string) {
  return useQuery<ICourseSummaryResponse, Error>({
    ...
    queryFn: () =>
      gsGet<ICourseSummaryResponse>(
        withStudentId(
          `/learning-analytics/courses/${courseId}/summary`,
          studentId,
        ),
      ),
    enabled: !!courseId,
...
export function useEmotionDistribution(courseId: string, studentId?: string) {
  return useQuery<IEmotionDistributionResponse, Error>({
    ...
    queryFn: () =>
      gsGet<IEmotionDistributionResponse>(
        withStudentId(
          `/learning-analytics/courses/${courseId}/emotion-distributions`,
          studentId,
        ),
      ),
    enabled: !!courseId,
...
export function useEmotionDistributionOverall(courseId: string) {
  return useQuery<IEmotionDistributionClassOverallResponse, Error>({
    ...
    queryFn: () =>
      gsGet<IEmotionDistributionClassOverallResponse>(
        `/learning-analytics/courses/${courseId}/class-overall-emotion`,
      ),
    enabled: !!courseId,
    staleTime: 2 * 60 * 1000,
  });
...
export function useStudyTimeByModule(courseId: string, studentId?: string) {
  return useQuery<IStudyTimeByModuleResponse, Error>({
    ...
    queryFn: () =>
      gsGet<IStudyTimeByModuleResponse>(
        withStudentId(
          `/learning-analytics/courses/${courseId}/material-study-times`,
          studentId,
        ),
      ),
    enabled: !!courseId,
...
export function useActivityLogs(
  ...
  return useQuery<IActivityLogResponse, Error>({
    queryKey: queryKeys.lad.activityLogs(
      courseId,
      studentId || "",
      page,
      limit,
...
export function useDiagnosticTestDistribution(courseId: string) {
  return useQuery<IDistributionDiagnosticTestResponse, Error>({
    ...
    queryFn: () =>
      gsGet<IDistributionDiagnosticTestResponse>(
        `/learning-analytics/courses/${courseId}/class-diagnostic-distribution`,
      ),
    enabled: !!courseId,
    staleTime: 2 * 60 * 1000,
  });
...
export function useRemedialTestDistribution(courseId: string) {
  return useQuery<IDistributionRemedialResponse, Error>({
    ...
    queryFn: () =>
      gsGet<IDistributionRemedialResponse>(
        `/learning-analytics/courses/${courseId}/class-remedial-distribution`,
      ),
    enabled: !!courseId,
    staleTime: 2 * 60 * 1000,
  });
...
export function useModuleProgressTable(
  courseModuleId: string,
  studentId?: string,
) {
  return useQuery<IModuleProgressTableResponse, Error>({
    queryKey: queryKeys.lad.moduleProgressTable(
      courseModuleId,
      studentId || "",
    ),
    queryFn: () =>
      gsGet<IModuleProgressTableResponse>(
        withStudentId(
          `/learning-analytics/modules/${courseModuleId}/progress-table`,
          studentId,
        ),
      ),
...
export function useExportActivityLogs() {
  ...
  >({
    mutationFn: ({
      ...
      return gsDownloadFile(
        `/learning-analytics/courses/${courseId}/activity-logs/export${qs ? `?${qs}` : ""}`,
      );
    },
  });
}
...
export function useDownloadAnalytics() {
  ...
  >({
    mutationFn: (courseSlug: string) =>
      gsDownloadFile(
        `/learning-analytics/courses/${courseSlug}/download-analytics`,
      ),
  });
}
...
export function useDownloadModuleAnalytics() {
  ...
  >({
    mutationFn: (courseModuleId: string) =>
      gsDownloadFile(
        `/learning-analytics/modules/${courseModuleId}/download-analytics`,
      ),
  });
}
...
export function useWordCloudForum(courseId: string, limit?: number) {
  ...
  return useQuery<IWordCloudForumData, Error>({
    ...
    queryFn: () =>
      gsGet<IWordCloudForumData>(
        `/learning-analytics/courses/${courseId}/word-cloud${queryStr}`,
      ),
    enabled: !!courseId,
    staleTime: 2 * 60 * 1000,
  });
