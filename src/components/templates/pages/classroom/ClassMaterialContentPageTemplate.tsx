...
export default function ClassMaterialContentPageTemplate({
  ...
  const [selectedStepId, setSelectedStepId] = useState<string | null>(
    MATERIAL_MODULES.find((module) => module.steps)?.steps?.[0]?.id ?? null,
  );
  ...
  const moduleSteps =
    MATERIAL_MODULES.find((module) => module.steps)?.steps ?? [];
  const selectedStepIndex = moduleSteps.findIndex(
    (step) => step.id === selectedStepId,
  );
  const stepCount = moduleSteps.length;
  const prevStep = moduleSteps[selectedStepIndex - 1];
  ...
  const getNavLabel = (direction: "prev" | "next") => {
    ...
    if (!step) {
      return direction === "prev" ? "Sebelumnya" : "Selanjutnya";
    }
  ...
  const handleStepNavigation = (direction: "prev" | "next") => {
    ...
    if (nextIndex >= 0 && nextIndex < stepCount) {
      setSelectedStepId(moduleSteps[nextIndex].id);
    }
  };

  return (
    <section className="min-h-screen rounded-3xl sm:p-3 lg:p-0">
      <div className="mb-3 flex flex-wrap items-center gap-2 text-sm text-[#64748B]">
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px] xl:items-start">
        <div>
          <div className="overflow-hidden rounded-lg   ">
            <div className="flex flex-wrap items-center justify-between gap-3 bg-[#1F2E46] px-3 py-3 text-xs text-white/90 sm:px-4">
              <div className="inline-flex items-center gap-3">
                {(() => {
                  const ActiveContentIcon = getActiveContentIcon(
                    activeStep?.typeLabel ?? "Materi",
                  );

                  return (
                    <ActiveContentIcon className="h-4 w-4 text-white/80" />
                  );
                })()}
                <div>
              ...
              <div className="">
                {isVideoMode ? (
                  <>
                    ...
                    <div className="mt-5 rounded-[28px] border border-[#E2E8F0] bg-white p-6 shadow-sm">
                      ...
                      <p className="mt-4 text-sm leading-6 text-[#475569]">
                        Tonton video ini untuk mendapatkan pemahaman visual
                        tentang konsep variabel dan bagaimana menyelesaikan
                        persamaan kuadrat.
                      </p>
                      <div className="mt-6 h-2 overflow-hidden rounded-full bg-[#E2E8F0]">
                        <div className="h-full w-10/12 rounded-full bg-[#2563EB]" />
                      </div>
                    ...
                    <div className="mt-4 rounded-3xl border border-[#DBEAFE] bg-[#EFF6FF] p-4 text-sm text-[#1E293B]">
                      <div className="flex items-start gap-3">
                        <span className="inline-flex h-9 w-9 items-center justify-center rounded-2xl bg-[#DBEAFE] text-[#2563EB]">
                          <svg
                            viewBox="0 0 24 24"
                            className="h-5 w-5"
                            fill="currentColor"
                        ...
                        <p className="leading-6">
                          ...
                          untuk mengerjakan{" "}
                          {nextStep?.typeLabel ?? "langkah berikutnya"}.
                        </p>
                      </div>
                    </div>
                  </>
                  ...
                  <>
                    <div className="mb-4 rounded-3xl border border-[#E2E8F0] bg-white p-4 shadow-sm sm:p-5">
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-[#64748B]">
                            Siapkan lembar kerja digital untuk menjawab soal dan
                            melatih pemahaman materi.
                          </p>
                        </div>
                      </div>
                    </div>
                    <div className="mt-4 rounded-lg border border-dashed border-[#CBD5E1] bg-[#F8FAFC] p-4">
                      <p className="text-sm font-medium text-[#334155]">
                        E-LKPD ini dapat digunakan untuk latihan konsep.
                      </p>
                      <p className="mt-1 text-sm text-[#64748B]">
                        Gunakan area ini sebagai placeholder sebelum konten
                        E-LKPD final dari backend/API ditambahkan.
                      </p>
                    </div>
                  </>
                ) : isTestDiagnosisMode ? (
                  <>
                    <div className="mb-4 rounded-3xl border border-[#E2E8F0] bg-white p-4 shadow-sm sm:p-5">
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-[#64748B]">
                            Uji pemahamanmu dengan soal diagnosis yang
                            disesuaikan untuk langkah ini.
                          </p>
                        </div>
                      </div>
                    </div>
                    <div className="mt-4 rounded-lg border border-dashed border-[#CBD5E1] bg-[#F8FAFC] p-4">
                      <p className="text-sm font-medium text-[#334155]">
                        Tes diagnosis siap digunakan setelah video dan E-LKPD.
                      </p>
                      <p className="mt-1 text-sm text-[#64748B]">
                        Gunakan area ini sebagai placeholder sebelum konten tes
                        final dari backend/API ditambahkan.
                      </p>
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div className="mt-4 rounded-lg border border-dashed border-[#CBD5E1] bg-[#F8FAFC] p-4">
                      ...
                      <p className="mt-1 text-sm text-[#64748B]">
                        Gunakan area ini sebagai placeholder sebelum materi
                        final dari backend/API dimasukkan.
                      </p>
                    </div>
                  </>
                )}
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <div
              className={cn(
                ...
                !isPdfMode && "justify-center w-full",
              )}
            >
              {isPdfMode ? (
                <>
        ...
        <aside className="sticky top-4 h-fit rounded-xl border border-[#E2E8F0] bg-white p-4 sm:p-5">
